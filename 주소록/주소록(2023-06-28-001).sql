...
ANSI SQL : 모든 DBMS SW 에 거의 공통으로 구현된 명령 문법
    표준화된 문법으로 DBMS SW 가 달라고 사용할 수 있는 문법
...
-- 코딩(java.. 등등)과 연동하여 사용할때는 가급적 ALL Project 을 사용하는 것이 좋다
SELECT a_id, a_name, a_tel, a_ddr
FROM tbl_address;

-- ID 가 A0010 이하인 데이터들만 보여달라
...
SELECTION : 특정 조건에 맞는 데이터를 SELECT 하는 것
    WHERE 절에 조건식 부여하여 리스트 제헌 : SELECT * FROM [table] WHERE 조건
*/
