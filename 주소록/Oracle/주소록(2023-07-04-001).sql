...
제1정규화가 된 치미 데이터는 취미1, 취미2, 취미3 칼럼이 만들어진 table 이다
이 테이블에 만약 4번째 취미를 추가하고 싶으면, 새로운 칼럼을 추가하거나
    4번째 취미를 다른 취미항목으로 교체 해야 한다.
    
    만약 취미항목이 늙어나 칼럼을 추가하게 되면
    Table 구조를 변경해야되고, 상당히 많은 비용이 요구된다(시간 등)
...
2,3 정규화 된 Table 은 필연적으로 Table 간의 Join 이 되어야만 정보를 명확히 알수 있다
JOIN 은 2개 이상의 Table 을 연계하여 필요한 정보를 확인하는 것이다
*/
...
-- LEFT JOIN
-- JOIN 을 실행하는 Table 같에 참조무결성이 의심스럽거나 위배될지 모른다는
-- 전재하에 데이터를 확인하는 JOIN
-- FROM [table1] LEFT JOIN [table2]
-- 왼쪽(LEFT)의 table 데이터는 무조건 SELECT 하고
-- 오른쪽의 table2 데이터는 ON 에서 설정한 조건에 맞는것만 가져오기
-- 오른쪽의 table2 에 조건에 맞는 데이터가 있으면 보여주고
--       없으면 null 표현
-- 1. SELECT 한 데이터의 누락이 발생하지 않는다
-- 2. 참조 무결성 제약 조건이 완전하게 성립하는지 확인하는 용도
