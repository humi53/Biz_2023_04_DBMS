...
-- tbl_buyer TABLE 에 저장된 데이터 중에서 buname(이름) gkdahrdml epdlxjrk
...
-- 데이터가 없다 이런경우 Result(결과) 가 Empty 인 상태 이다
...
TABLE 을 작성하기 앞서, 개념석, 논리적 모델링을 수행하는데 이 단계에서는 TABLE을
Entity 라고 한다
...
-- Projection : SELECT 조회를 할 때 별표(*)를 사용하지 않고 칼럼을 나열하는것
...
/*
 ...
 SQL 에서 데이터를 INSERT, UPDATE, DELETE 를 수행하는 경우
    추가, 수정, 삭제된 정보는 아직 storage 에 반영되지 않고, 메모리에 임시 보관(저장)된 상태이다
 ...
 SQL 에서 INSERT, UPDATE, DELETE 를 수행한 다음에는 강제로 storage 에 Commit 을
 해주어야 한다
...
-- PK 칼럼을 기준으로 조회하면 데이터 없거나, 1개만 조회된다.
SELECT buid, buname, buaddr, bubirth, bujob
FROM tbl_buyer
WHERE buid = '0001';

INSERT INTO tbl_buyer(buid, buname, butel)
VALUES('0001', '이', '010-000-0000');
