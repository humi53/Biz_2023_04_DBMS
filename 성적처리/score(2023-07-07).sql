...
INSERT INTO [테이블명]([컬럼들]) VALUES([컬럼값들]);
...
SHOW CREATE TABLE tbl_score;

ALTER TABLE tbl_score
ADD FOREIGN KEY (sc_bcode) REFERENCES tbl_subject(b_code);
...
-- 제3정규화가 되어있는 데이터를 PIVOT 보고서 형식으로 출력
SELECT sc_stnum,
	SUM(IF(sc_bcode = 'B001', sc_score, 0)) AS 국어,
    SUM(IF(sc_bcode = 'B002', sc_score, 0)) AS 영어,
    SUM(IF(sc_bcode = 'B003', sc_score, 0)) AS 수학,
...
-- 조건절(WHERE, HAVING)에 SUB Query 는 결과가 반드시
-- 한개 이하여야 한다
SELECT sc_stnum, SUM(sc_score) AS 총점
FROM tbl_score
GROUP BY sc_stnum
HAVING 총점 < 
	(SELECT SUM(sc_score) AS 총점
	FROM tbl_score
